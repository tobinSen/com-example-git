package com.example.spring.jedis.test.shardedJedis;
...
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
	...
	private List<HostAndPort> initSentinels(Set<String> sentinels, final List<String> masters) {
	    ...
	    for (String masterName : masters) {
            ...
            /**
             * 最终的效果是：
			 * 	1、首先遍历每个 MasterName 名称，让后遍历所有的 sentinel 监听到的 master 的节点信息
			 * 	2、最后的效果是：sentinel 只会去监听一个 master 节点 ，然后去搜集这些节点的 HostAndPort 映射新
			 */
			while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
	    		...
	    		for (String sentinel : sentinels) {
					...
					try {
					    ...
					    if (master == null) {
					    	// 用sentinel节点来获取，获取这个sentinel监听对应的master节点
					    	...
					    	if (hostAndPort != null && hostAndPort.size() > 0) {
								...
								// 通过 sentinel 来获取对应监听的 master 节点的信息
								shardMasters.add(master);
								// <masterName,HostAndPort>
		    	...
		    	// 遍历所有的 sentinel 都没有寻找到，这里就村子是否需要重试进行获取
		    	if (null == master) {
		    		try {
	    ...
	    if (masters.size() != 0 && masters.size() == shardMasters.size()) {
			...
			for (String sentinel : sentinels) {
			    final HostAndPort hap = toHostAndPort(Arrays.asList(sentinel.split(":")));
			    // 所有的 Masters 节点对应一个 sentinel 节点信息
