...
public class ShardedJedisSentinelPoolExt extends Pool<ShardedJedis> {
    ...
    public ShardedJedisSentinelPoolExt(List<String> masters, Set<String> sentinels) {
        this(masters, sentinels, new GenericObjectPoolConfig(),
                Protocol.DEFAULT_TIMEOUT, null, Protocol.DEFAULT_DATABASE);
    }

    public ShardedJedisSentinelPoolExt(List<String> masters, Set<String> sentinels, String password) {
        this(masters, sentinels, new GenericObjectPoolConfig(),
                Protocol.DEFAULT_TIMEOUT, password);
    }

    public ShardedJedisSentinelPoolExt(final GenericObjectPoolConfig poolConfig, List<String> masters, Set<String> sentinels) {
    ...
    private List<HostAndPort> initSentinels(Set<String> sentinels, final List<String> masters) {
        ...
        for (String masterName : masters) {
            ...
            if (!fetched && sentinelRetry >= MAX_RETRY_SENTINEL) {
                log.warn("All sentinels down and try " + MAX_RETRY_SENTINEL + " times, Abort.");
            ...
            while (running.get()) {
                ...
                try {
                    jedis.subscribe(new JedisPubSubAdapter() {
                        @Override
                        public void onMessage(String channel, String message) {
                            log.warn("Sentinel " + host + ":" + port + " published: " + message + ".");
